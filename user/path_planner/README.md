...
2. 激光读数过 3 点中值滤波(偶然探测错误/震动毛刺/单帧超远距离被剔除,
   连续坏帧才停机);
3. 瞬态故障(位姿/IMU/激光/电机/前激光障碍)停机后,健康 1s **自动重布防**
   (前激光急停要求障碍清除到 >14cm 才恢复,避免停-启循环),最多 3 次;
4. 所有停机走 VESC SET_BRAKE 锁存;ARRIVED/STOPPED 后遥控可直接接管;
5. 控制只用融合位姿;位姿缺 150ms 降速 0.3m/s、800ms 停机;
6. 轨迹进入 RUN 前强校验:首点=本次起点、末点=目标(±容差)、
   曲率/净距/间距全部达标,不通过 STOP_BUILD 不出车;
7. 起步朝向 ±30° 只在**冷启动**检查一次;瞬态故障恢复重布防时
   不复查(故障期间车身偏了也继续跑,由 yaw-lock 转回)。
