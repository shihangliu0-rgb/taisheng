...
/* =========================================================================
 *  板级接线(按本工程实际硬件) —— 引脚以硬件为准
 *  -------------------------------------------------------------------------
 *   SPI1 : PA5 = SCK, PA6 = MISO, PA7 = MOSI   (需在 CubeMX 开 SPI1 + DMA)
 *   CS#  : PA4 (软件片选，GPIO 推挽输出，不用硬件 NSS)
 *   注：本驱动 handle 化、SPI 句柄与 CS 引脚都由 W25Q_Init() 传入，不写死。
 *       SPI1 的使能(生成 spi.c/hspi1 + PA5/6/7 复用 + DMA RX/TX + PA4 输出)
 *       由用户在 CubeMX 完成，本驱动不碰 spi.c/gpio.c。
 *
 *   初始化示例(开好 SPI1 后):
 *     W25Q_HandleTypeDef hflash;
 *     W25Q_Init(&hflash, &hspi1, GPIOA, GPIO_PIN_4);   // CS = PA4
 *     W25Q_EraseSector(&hflash, 0); W25Q_Write(&hflash, 0, buf, 256);
 * ========================================================================= */
