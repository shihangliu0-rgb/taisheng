...
/* RS485 方向手动控制(723 主板，与 F405 工程不同)：
 *   PD10、PD11 同时拉高 = 发送(DE 使能)；同时拉低 = 接收(RE 使能)。
 *   注意：PD10/PD11 需在 CubeMX 里配为推挽输出(基础配置由用户决定，不在本工程改动 gpio.c)。
 *   默认(空闲)保持低电平=接收；仅在 Imu_SendCommand 发送期间临时拉高。 */
#define IMU_RS485_DE_PORT   GPIOD
#define IMU_RS485_DE_PIN    (GPIO_PIN_10 | GPIO_PIN_11)
...
/**
 * @brief  绑定 IMU 传感器所用串口句柄（移植自 F405：原硬编码 USART6，现参数化）
 * @note   不修改 usart.c；具体用哪个串口由 CubeMX 开启后传入（见 imu_main 的 IMU_UART_HANDLE）
 * @param  uart 已完成初始化的串口句柄
 * @retval None
 */
void Imu_AttachUart(UART_HandleTypeDef *uart);

/**
 * @brief  初始化 IMU 模块与卡尔曼滤波器，并启动 DMA 接收
 * @note   调用前须先 Imu_AttachUart() 绑定串口
 * @retval None
 */
void Imu_Init(void);
