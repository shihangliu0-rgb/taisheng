...
/* 板级日志层(my_main)已随 F405 板级代码一并剥离；此处置为空操作。
 * 如需日志，可改写为 Upper_SendLog() 等，不在此引入新的板级依赖。 */
...
static UART_HandleTypeDef *imu_uart = 0;   /* 移植自 F405：原硬编码 &huart6，现参数化 */
...
static void Imu_SendCommand(const uint8_t *cmd, uint16_t len)
{
    ...
    /* RS485 方向现已由硬件自动控制(自动收发切换)，无需手动拉 DE/RE */
...
/**
 * @brief  初始化 IMU 模块与算法，并启动 DMA 接收
 * @note   调用前须先 Imu_AttachUart() 绑定串口；未绑定则跳过启动接收
 * @retval None
 */
void Imu_Init(void)
{
...
void Imu_StartReceive(void)
{
    ...
    if (HAL_UARTEx_ReceiveToIdle_DMA(imu_uart, imu_rx_dma_buffer, sizeof(imu_rx_dma_buffer)) == HAL_OK)
    {
