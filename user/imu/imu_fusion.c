...
#include "imu_main.h"       /* ImuMain_GetData: yaw / 世界系惯导速度 */
...
static bool read_encoder(uint32_t now_ms)
{
    ...
    /* 经 IMU 偏航角转世界系 */
    {
        ...
        if (ImuMain_GetData(&imu) && imu.yaw_valid)
        {
            yaw_deg = imu.yaw_deg;
        }
        yaw_rad = yaw_deg * FUSION_DEG2RAD * FUSION_YAW_SIGN;
    }
    c = cosf(yaw_rad);
    s = sinf(yaw_rad);
...
void ImuFusion_Update(uint32_t now_ms)
{
    ...
    /* -------- 融合：fused = w_enc*enc + (1-w_enc)*imu -------- */
    {
        ...
        if (ImuMain_GetData(&imu) && imu.pos_valid)
        {
            imu_vx = imu.vel_world_x_mps;
            imu_vy = imu.vel_world_y_mps;
        }
        else
        {
            /* 惯导尚未就绪(未收到加速度帧/未归零)，此时只信编码器 */
            imu_vx = 0.0f;
            imu_vy = 0.0f;
        }
    }
