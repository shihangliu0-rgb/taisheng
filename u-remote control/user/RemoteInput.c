...
void Remote_Update(void)
{
    ...
    for (i = 0U; i < REMOTE_SWITCH_COUNT; i++)
    {
        remote_data.switch_state[i] = Remote_ReadLow(remote_sw_ports[i], remote_sw_pins[i]);
        if (remote_data.switch_state[i] != remote_last_sw[i])
        {
            remote_last_sw[i] = remote_data.switch_state[i];
            changed = 1U;
        }
    }
    for (i = 0U; i < REMOTE_KEY_COUNT; i++)
    {
        remote_data.key_state[i] = Remote_ReadLow(remote_key_ports[i], remote_key_pins[i]);
        if (remote_data.key_state[i] != remote_last_key[i])
        {
            remote_last_key[i] = remote_data.key_state[i];
            changed = 1U;
        }
    }
    if (changed != 0U)
